select order_id , warehouse_id from Shipments where order_id 
in (select order_id from Orders where customer_id in 
(select cust_id from Customer where cname like "%kumar%"))
